...
public class GEToolBar extends JToolBar {
	...
	public GEToolBar(String label) {
		...
		for (EToolBarButtons button : EToolBarButtons.values()) {
			...
			rButton.setSelectedIcon(
					new ImageIcon(GEConstants.IMG_URL + button.toString() + GEConstants.TOOLBAR_BTN_SLT));
			rButton.setActionCommand(button.name());
			rButton.addActionListener(toolBarHandler);
			// System.out.println("button.name : " + button.name() + ", button.toString() :
			// " + button.toString());
			this.add(rButton);
